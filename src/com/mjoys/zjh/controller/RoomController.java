...
public class RoomController extends IController {
	...
	public class QuickStartController extends IController implements
			DataListener<String> {
		...
		public void onData(SocketIOClient arg0, String arg1, AckRequest arg2)
			...
			// 快速查找空房间 / 创建一个空房间
			TableController newTc = TableCollector.getInstance().quickStart(
					this.server, user, arg0);
			arg0.set(G.CACHE_TABLE_CONTROLLER, newTc);
			// 向客户端发送房间数据
			newTc.broadcast(CSMapping.S2C.MATCHED_TABLE_SUCCESS);
		}
	}

	public class JoinRoomController extends IController implements
			DataListener<String> {
		...
		public void onData(SocketIOClient arg0, String arg1, AckRequest arg2)
			...
			TableController newTc = TableCollector.getInstance().joinTable(
					this.server, user, table.getTableID(), arg0);
			if (tc != null) {
				arg0.set(G.CACHE_TABLE_CONTROLLER, newTc);
				// 回执加入成功
				newTc.broadcast(CSMapping.S2C.MATCHED_TABLE_SUCCESS);
				...
				// 回执加入失败
				newTc.broadcast(CSMapping.S2C.MATCHED_TABLE_FAILED);

			}
		}
	}

	public class CreateController extends IController implements
			DataListener<String> {
		...
		public void onData(SocketIOClient arg0, String arg1, AckRequest arg2)
			...
			byte bytes[] = ProtobufUtility.toBytes(arg1);
			Table table = new Table(bytes);
			TableController newTc = TableCollector.getInstance().createTable(
					this.server, user, table, arg0);
			if (tc != null) {
				arg0.set(G.CACHE_TABLE_CONTROLLER, newTc);
				// 回执成功创建
				newTc.broadcast(CSMapping.S2C.CREATE_TABLE_SUCCESS);
			} else {
				// 回执创建失败
				newTc.broadcast(CSMapping.S2C.CREATE_TABLE_FAILED);
			}
		}
